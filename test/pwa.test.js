...
import { JS } from '../src/static.js';

/** Runs /app.js with a fake page and a controllable clock (only what auto-refresh needs). */
function loadAppJs({ onLine = true, activeElement = null } = {}) {
  const docHandlers = {};
  const winHandlers = {};
  let now = 1_000_000;
  const replaced = [];
  ...
  new Function('document', 'window', 'navigator', 'location', 'Date', JS)(document, window, { onLine }, location, { now: () => now });
...
test('auto-refresh: not after a short switch, not offline, not while typing', () => {
  const short = loadAppJs();
  short.setVisible(false);
  short.wait(2 * MIN);
  short.setVisible(true);
  assert.deepEqual(short.replaced, []);

  const offline = loadAppJs({ onLine: false });
  offline.setVisible(false);
  offline.wait(10 * MIN);
  offline.setVisible(true);
  assert.deepEqual(offline.replaced, []);

  const typing = loadAppJs({ activeElement: { tagName: 'INPUT', value: 'https://feed.x.test/rss' } });
...
/** Runs /app.js with one article (title link + Brave button) to test clicks. */
function loadArticlePage({ standalone }) {
  class Element {
  ...
  const article = new Element('article', [], { dataset: { id: '7' }, querySelectorAll: () => [] });
  const title = new Element('a', ['title'], { href: 'https://news.x.test/a?b=1&c', parent: article });
  const brave = new Element('a', ['open-brave'], { href: 'https://news.x.test/a?b=1&c', parent: article, hidden: true });
  const handlers = {};
  const posted = [];
  const document = {
    hidden: false,
    addEventListener: (type, fn) => (handlers[type] = fn),
    querySelector: () => null,
    querySelectorAll: (s) => (s === 'a.open-brave' ? [brave] : []),
  };
  const window = { matchMedia: () => ({ matches: standalone }), addEventListener() {} };
  const location = { href: 'https://rss.x.test/', replace() {} };
  const fetch = async (url) => {
    posted.push(url);
    return { ok: true, json: async () => ({ is_read: 1, is_starred: 0, is_hidden: 0 }) };
  };
  new Function('document', 'window', 'navigator', 'location', 'Date', 'Element', 'fetch', JS)(
    document, window, { onLine: true }, location, Date, Element, fetch
  );
  ...
  return { article, title, brave, location, posted, click };
}

test('Brave button: visible only in the installed app', () => {
  assert.equal(loadArticlePage({ standalone: true }).brave.hidden, false);
  assert.equal(loadArticlePage({ standalone: false }).brave.hidden, true);
});

test('Brave button: opens the article in Brave and marks it read', async () => {
  const page = loadArticlePage({ standalone: true });
  assert.equal(await page.click(page.brave), true, 'default navigation replaced');
  assert.equal(page.location.href, 'brave://open-url?url=' + encodeURIComponent('https://news.x.test/a?b=1&c'));
  assert.deepEqual(page.posted, ['/articles/7/read']);
  assert.ok(page.article.classes.has('is-read'));
});

test('title click: opens normally and marks the article read (once)', async () => {
  const page = loadArticlePage({ standalone: true });
  assert.equal(await page.click(page.title), false, 'normal link navigation');
  assert.equal(page.location.href, 'https://rss.x.test/');
  await page.click(page.title);
  assert.deepEqual(page.posted, ['/articles/7/read'], 'already read → no second request');
});
