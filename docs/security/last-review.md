...
|---|---|---|---|
| 2026-09-26 | Diff review | since `73e2fd9` incl. uncommitted (client code moved to `src/client/` as ES modules, client store + live unread counts) | 1 Medium (request `?v=` reflected into served module imports → script gadget past the CSP) fixed before commit; test added for non-http Brave links |
...
**Next diff review:** changes since the commit that contains the client modules (the next commit after `73e2fd9`).
